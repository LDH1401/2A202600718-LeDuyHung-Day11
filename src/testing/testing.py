...
from attacks.attacks import adversarial_prompts, is_blocked_response, run_attacks
...
@dataclass
class TestResult:
    """Result of a single security test used to calculate leak metrics."""
    attack_id: int
    category: str
    input_text: str
    response: str
...
class SecurityTestPipeline:
    ...
    It repeatedly attacks an agent, classifies leaks, and creates metrics so
    guardrail changes can be regression-tested instead of judged manually.
    ...
    async def run_single(self, attack: dict) -> TestResult:
        ...
        try:
            ...
            blocked = is_blocked_response(response) or len(leaked) == 0
